    ...
    appAlert('Share', undefined, [
      {
        text: 'Camera',
        onPress: () => {
          // Nested menu rather than two flat top-level items — matches
          // the "one Camera entry, mode toggle between the two" shape
          // asked for. Both branches share pickImageBase64/pickVideoBase64
          // with the rest of this menu — same sendRich() pipeline, same
          // encryption, same limits — only the capture SOURCE differs
          // ('camera' vs the default 'library'); see media.ts.
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <PhotoSendPreview
        ...
        onSend={() => {
          if (pendingPhoto) sendRich(id ?? '', false, 'media', pendingPhoto);
