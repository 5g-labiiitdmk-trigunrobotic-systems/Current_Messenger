...
export default function AddPhoneScreen() {
  ...
  const onSubmit = async () => {
    if (!E164_RE.test(phone.trim())) {
      appAlert('Invalid phone number', 'Enter your number in international format, e.g. +14155550123.');
      return;
    }
    setLoading(true);
    try {
      ...
      appAlert('Could not send code', e?.message ?? 'Check the number and try again.');
    } finally {
      setLoading(false);
    }
  };
